  ...
  return (
    <div className="flex min-h-screen bg-slate-100 font-sans text-slate-900">
      ...
      <div className="flex-1 flex flex-col min-h-screen ml-64">
        <Header title="Master Data Asset Type" subtitle="Kelola tipe aset yang digunakan dalam pengajuan BODR" />
        <main className="flex-1 overflow-y-auto px-8 py-6 space-y-5">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Cari class atau nama type..." className="bg-white border border-slate-200 rounded-xl px-3 py-2 text-slate-900 text-xs font-normal focus:outline-none focus:border-blue-600 w-72 shadow-2xs"/>
          ...
          <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-2xs">
            <table className="w-full text-xs">
              <thead className="bg-slate-50 border-b border-slate-200">
                <tr>{["No","Class","Nama Type Asset","Deskripsi","Status","Aksi"].map(h=><th key={h} className="px-4 py-3 text-left text-[10px] font-semibold text-slate-500 uppercase tracking-wider">{h}</th>)}</tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                {filtered.map((item, idx)=>(
                  <tr key={item.id} className="hover:bg-slate-50 transition-colors">
                    <td className="px-4 py-3 text-slate-400 text-xs font-medium font-mono">{idx+1}</td>
                    <td className="px-4 py-3 font-mono font-semibold text-blue-600">{item.class}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-xs p-4">
          <div className="bg-slate-900 border border-slate-700/70 rounded-2xl shadow-2xl w-full max-w-md">
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-800">
              <h3 className="font-semibold text-slate-100 uppercase tracking-wide text-sm">{editing?"Edit Asset Type":"Tambah Asset Type"}</h3>
            ...
            <form onSubmit={handleSave} className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1"><label className="text-slate-400 font-semibold uppercase tracking-wider text-[10px]">Class *</label><input className={inputCls} value={fClass} onChange={e=>setFClass(e.target.value)} required placeholder="BLDG"/></div>
                <div className="space-y-1"><label className="text-slate-400 font-semibold uppercase tracking-wider text-[10px]">Nama Type *</label><input className={inputCls} value={fNamaType} onChange={e=>setFNamaType(e.target.value)} required placeholder="Building"/></div>
              </div>
