  ...
  return (
    <div className="flex min-h-screen bg-slate-100 font-sans text-slate-900">
      ...
      <div className="flex-1 flex flex-col min-h-screen ml-64">
        <Header title="Master Data Capex Type" subtitle="Kelola tipe/kategori CAPEX yang tersedia dalam sistem" />
          ...
          <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-2xs">
            <table className="w-full text-xs">
              <thead className="bg-slate-50 border-b border-slate-200">
                <tr>{["No","Kode","Nama Type","Deskripsi","Status","Aksi"].map(h=><th key={h} className="px-4 py-3 text-left text-[10px] font-semibold text-slate-500 uppercase tracking-wider">{h}</th>)}</tr>
              </thead>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-xs p-4">
          <div className="bg-slate-900 border border-slate-700/70 rounded-2xl shadow-2xl w-full max-w-md">
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-800">
              <h3 className="font-semibold text-slate-100 uppercase tracking-wide text-sm">{editing?"Edit Capex Type":"Tambah Capex Type"}</h3>
            ...
            <form onSubmit={handleSave} className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1"><label className="text-slate-400 font-semibold uppercase tracking-wider text-[10px]">Kode *</label><input className={inputCls} value={fKode} onChange={e=>setFKode(e.target.value)} required/></div>
                <div className="space-y-1"><label className="text-slate-400 font-semibold uppercase tracking-wider text-[10px]">Nama *</label><input className={inputCls} value={fNama} onChange={e=>setFNama(e.target.value)} required/></div>
              </div>
