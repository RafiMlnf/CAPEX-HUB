...
import { ApiCapexType, api } from "../../lib/api";
...
export default function CapexTypePage() {
  const [items, setItems] = useState<ApiCapexType[]>([]);
  const [search, setSearch] = useState("");
  const [isOpen, setIsOpen] = useState(false);
  const [editing, setEditing] = useState<ApiCapexType | null>(null);
  ...
  const refresh = () => {
    api.getCapexTypes().then(setItems).catch(console.error);
  };

  useEffect(() => { refresh(); }, []);

  const openCreate = () => { setEditing(null); setFKode(""); setFNama(""); setFDeskripsi(""); setFStatus("active"); setIsOpen(true); };
  const openEdit = (item: ApiCapexType) => { setEditing(item); setFKode(item.kode); setFNama(item.nama); setFDeskripsi(item.deskripsi); setFStatus(item.status); setIsOpen(true); };
  const handleSave = async (e: React.FormEvent) => {
    e.preventDefault();
    const payload = { kode: fKode, nama: fNama, deskripsi: fDeskripsi, status: fStatus };
    if (editing) {
      await api.updateCapexType(editing.id, payload);
    } else {
      await api.createCapexType(payload);
    }
  ...
  return (
    <div className="flex min-h-screen bg-slate-100 font-sans text-slate-800">
      ...
      <div className="flex-1 flex flex-col min-h-screen ml-64 bg-slate-100">
        <Header title="Master Capex Type" subtitle="Kelola tipe-tipe capex untuk formulir BODR" />
        <main className="flex-1 overflow-y-auto px-8 py-6 space-y-5">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Cari tipe capex..." className="bg-white border border-slate-200 rounded-xl px-3 py-2 text-slate-800 text-xs focus:outline-none focus:border-blue-500 w-72 shadow-2xs font-normal"/>
              <span className="text-xs text-slate-500 font-semibold">{filtered.length} data</span>
            </div>
            <button onClick={openCreate} className="flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white font-semibold text-xs rounded-xl shadow-2xs cursor-pointer transition-all active:scale-95"><svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M12 4v16m8-8H4"/></svg>Tambah Capex Type</button>
          </div>
          <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-2xs">
            <table className="w-full text-xs">
              <thead className="bg-slate-50 border-b border-slate-200 text-slate-600 font-semibold uppercase tracking-wider text-[10px]">
                <tr>{["No","Kode","Nama","Deskripsi","Status","Aksi"].map(h=><th key={h} className="px-4 py-3 text-left">{h}</th>)}</tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                ...
                {filtered.length===0&&<tr><td colSpan={6} className="text-center text-slate-400 py-12 text-xs font-normal">Belum ada data Capex Type</td></tr>}
              </tbody>
            </table>
          </div>
        </main>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-xs p-4">
          <div className="bg-white border border-slate-200 rounded-2xl shadow-2xl w-full max-w-md overflow-hidden text-slate-800">
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 bg-slate-50/80">
              <h3 className="font-bold text-slate-800 uppercase tracking-wide text-sm">{editing?"Edit Capex Type":"Tambah Capex Type"}</h3>
            ...
            <form onSubmit={handleSave} className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1"><label className="text-slate-600 font-semibold uppercase tracking-wider text-[10px]">Kode *</label><input className={inputCls} value={fKode} onChange={e=>setFKode(e.target.value)} required placeholder="Contoh: CT-01"/></div>
                <div className="space-y-1"><label className="text-slate-600 font-semibold uppercase tracking-wider text-[10px]">Nama *</label><input className={inputCls} value={fNama} onChange={e=>setFNama(e.target.value)} required placeholder="Nama type"/></div>
              </div>
              <div className="space-y-1"><label className="text-slate-600 font-semibold uppercase tracking-wider text-[10px]">Deskripsi</label><textarea className={inputCls} value={fDeskripsi} onChange={e=>setFDeskripsi(e.target.value)} rows={2} placeholder="Deskripsi capex type..."/></div>
